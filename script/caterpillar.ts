...
const selectedSymbol: keyof typeof SYMBOLS = "tataMotors";
const data_path = path.join(BASE_DIR, "data", "raw", "1min", `${selectedSymbol}_ohlcv.json`);
...
async function downloadOHLCVData() {
  const instrumentKey = encodeURIComponent(SYMBOLS[selectedSymbol]!);
